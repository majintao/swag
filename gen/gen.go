...
// Build builds swagger json file  for given searchDir and mainAPIFile. Returns json
func (g *Gen) BuildSwagger(config *Config) (s string, err error) {
	if _, err := os.Stat(config.SearchDir); os.IsNotExist(err) {
		return s, fmt.Errorf("dir: %s is not exist", config.SearchDir)
	}
	...
	y, err := g.jsonToYAML(b)
	if err != nil {
		return s, fmt.Errorf("cannot convert json to yaml error: %s", err)
	}
	**/
